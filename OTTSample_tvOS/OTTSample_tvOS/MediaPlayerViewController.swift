...
class MediaPlayerViewController: UIViewController {
    ...
    @objc private func seekSwipe(recognizer: UISwipeGestureRecognizer) {
        ...
        if kalturaOTTPlayer.isPlaying { return }
        ...
        if mediaProgressView.progress != Float(kalturaOTTPlayer.currentTime / kalturaOTTPlayer.duration) {
        ...
        kalturaOTTPlayer.addObserver(self, events: [KPPlayerEvent.loadedMetadata, KPPlayerEvent.ended, KPPlayerEvent.play, KPPlayerEvent.playing, KPPlayerEvent.pause, KPPlayerEvent.canPlay, KPPlayerEvent.seeking, KPPlayerEvent.seeked, KPPlayerEvent.playbackStalled, KPPlayerEvent.stateChanged]) { [weak self] event in
            ...
            DispatchQueue.main.async {
                ...
                case is KPPlayerEvent.Seeked:
                    ...
                    if self.kalturaOTTPlayer.currentTime < self.kalturaOTTPlayer.duration {
                        self.kalturaOTTPlayer.play()
                    }
    ...
    private func handleProgress() {
        kalturaOTTPlayer.addObserver(self, events: [KPPlayerEvent.playheadUpdate]) { [weak self] event in
            ...
            DispatchQueue.main.async {
                ...
                self.mediaProgressView.progress = Float(currentTimeNumber.doubleValue / self.kalturaOTTPlayer.duration)
            }
        }
    }
    ...
    private func handleBufferedProgress() {
        kalturaOTTPlayer.addObserver(self, event: KPPlayerEvent.loadedTimeRanges) { [weak self] event in
            ...
            DispatchQueue.main.async {
                self.mediaProgressView.bufferValue = Float(self.kalturaOTTPlayer.bufferedTime / self.kalturaOTTPlayer.duration)
            }
        }
    }
...
extension MediaPlayerViewController {
    ...
    @IBAction private func playButtonTouched(_ button: UIButton) {
        if userSeekInProgress {
            let duration = kalturaOTTPlayer.duration
            let currentProgress = Float(kalturaOTTPlayer.currentTime / duration)
            
            if mediaProgressView.progress != currentProgress {
                let seekToTime = mediaProgressView.progress * Float(duration)
                kalturaOTTPlayer.seek(to: TimeInterval(seekToTime))
            } else {
                userSeekInProgress = false
            }
        } else {
