...
class MediaPlayerViewController: UIViewController {
    ...
    var videoData: VideoData? {
        didSet {
            if viewIfLoaded != nil, let videoData = self.videoData {
                ...
                kalturaOVPPlayer.updatePlayerOptions(ovpPlayerOptions)
            }
        }
    }
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        kalturaOVPPlayer.removeObserver(self, events: KPPlayerEvent.allEventTypes)
        // IMA
        kalturaOVPPlayer.removeObserver(self, events: KPAdEvent.allEventTypes)
    }
    ...
    @objc private func seekSwipe(recognizer: UISwipeGestureRecognizer) {
        ...
        if kalturaOVPPlayer.isPlaying { return }
        ...
        if mediaProgressView.progress != Float(kalturaOVPPlayer.currentTime / kalturaOVPPlayer.duration) {
    ...
    private func registerPlaybackEvents() {
        kalturaOVPPlayer.addObserver(self, events: [KPPlayerEvent.loadedMetadata, KPPlayerEvent.ended, KPPlayerEvent.play, KPPlayerEvent.playing, KPPlayerEvent.pause, KPPlayerEvent.canPlay, KPPlayerEvent.seeking, KPPlayerEvent.seeked, KPPlayerEvent.playbackStalled, KPPlayerEvent.stateChanged]) { [weak self] event in
            guard let self = self else { return }
            ...
            DispatchQueue.main.async {
                ...
                case is KPPlayerEvent.LoadedMetadata:
                    if self.kalturaOVPPlayer.isLive() {
                ...
                case is KPPlayerEvent.Seeking:
                    ...
                    if self.kalturaOVPPlayer.currentTime < self.kalturaOVPPlayer.duration, self.playPauseButton.displayState == .replay {
                        self.playPauseButton.displayState = .play
                    }
                case is KPPlayerEvent.Seeked:
                    ...
                    if self.kalturaOVPPlayer.currentTime < self.kalturaOVPPlayer.duration {
                        self.kalturaOVPPlayer.play()
                    }
    ...
    private func handleTracks() {
        kalturaOVPPlayer.addObserver(self, events: [KPPlayerEvent.tracksAvailable]) { [weak self] event in
            guard let self = self else { return }
    ...
    private func handleProgress() {
        kalturaOVPPlayer.addObserver(self, events: [KPPlayerEvent.playheadUpdate]) { [weak self] event in
            ...
            DispatchQueue.main.async {
                ...
                self.mediaProgressView.progress = Float(currentTimeNumber.doubleValue / self.kalturaOVPPlayer.duration)
            }
        }
    }
    ...
    private func handleBufferedProgress() {
        kalturaOVPPlayer.addObserver(self, event: KPPlayerEvent.loadedTimeRanges) { [weak self] event in
            ...
            DispatchQueue.main.async {
                self.mediaProgressView.bufferValue = Float(self.kalturaOVPPlayer.bufferedTime / self.kalturaOVPPlayer.duration)
            }
        }
    }
    
    private func handleDuration() {
        kalturaOVPPlayer.addObserver(self, events: [KPPlayerEvent.durationChanged]) { [weak self] event in
            guard let self = self else { return }
            
            let duration = self.getTimeRepresentation(self.kalturaOVPPlayer.duration)
            DispatchQueue.main.async {
    ...
    private func handleError() {
        kalturaOVPPlayer.addObserver(self, events: [KPPlayerEvent.error]) { [weak self] event in
            guard let self = self else { return }
            DispatchQueue.main.async {
                self.activityIndicator.stopAnimating()
    ...
    private func registerAdEvents() {
        kalturaOVPPlayer.addObserver(self, events: [KPAdEvent.adLoaded, KPAdEvent.adPaused, KPAdEvent.adResumed, KPAdEvent.adStartedBuffering, KPAdEvent.adPlaybackReady, KPAdEvent.adStarted, KPAdEvent.adComplete, KPAdEvent.adSkipped, KPAdEvent.allAdsCompleted]) { [weak self] adEvent in
            guard let self = self else { return }
...
extension MediaPlayerViewController {
    ...
    @IBAction private func speechTouched(_ button: UIButton) {
        ...
        for track in tracks {
            alertController.addAction(UIAlertAction(title: track.title, style: UIAlertAction.Style.default, handler: { (alertAction) in
                self.kalturaOVPPlayer.selectTrack(trackId: track.id)
            }))
        }
    ...
    @IBAction private func playButtonTouched(_ button: UIButton) {
        if userSeekInProgress {
            let duration = kalturaOVPPlayer.duration
            let currentProgress = Float(kalturaOVPPlayer.currentTime / duration)
            
            if mediaProgressView.progress != currentProgress {
                let seekToTime = mediaProgressView.progress * Float(duration)
                kalturaOVPPlayer.seek(to: TimeInterval(seekToTime))
            } else {
                userSeekInProgress = false
            }
        } else {
            if playPauseButton.displayState == .replay {
                kalturaOVPPlayer.replay()
                showPlayerControllers(false, delay: 0.5)
            } else if kalturaOVPPlayer.isPlaying || kalturaOVPPlayer.rate > 0 {
                kalturaOVPPlayer.pause()
            } else {
