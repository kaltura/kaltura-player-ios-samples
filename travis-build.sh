...
buildBasicSampleiOS() {
  ...
  cd BasicSample
  xcodebuild clean build -workspace BasicSample.xcworkspace -scheme BasicSample ONLY_ACTIVE_ARCH=NO -destination 'platform=iOS Simulator,name=iPhone X' | tee xcodebuild.log | xcpretty -r html && exit ${PIPESTATUS[0]}
  cd ../
}
